...
def get_args():
    ...
    # [ANDY] add arguments
    parser.add_argument("--regularizer",    default='cosine_sim', type=str)
    parser.add_argument("--num_extractors", default=5,            type=int)
    parser.add_argument("--alpha3",         default=0,         type=float)
...
class Trainer:
    def __init__(self, args, device):
        ...
        self.extractor = resnet18(args = self.args, classes=args.n_classes).to(device)
        self.convertor = AugNet(1).cuda()

#        summary(self.extractor, torch.zeros((64, 3, 224,224)).to(device)) 
        #summary(self.convertor, torch.zeros((64, 3, 224,224)).to(device)) 
    ...
    def _do_epoch(self, epoch=None):
        ...
        for it, ((data, _, class_l), _, idx) in enumerate(self.source_loader):
            ...
            # forward
            if self.args.ours:
                logits, tuple, regularize_loss = self.extractor(data_aug)
            else:
            ...
            # Total loss & backward
            # [ANDY] add regularize loss
            class_loss = criterion(logits, labels)
            if self.args.ours:
                loss = class_loss + self.args.alpha3*regularize_loss + self.args.alpha2*likeli + self.args.alpha1*con 
            else:
            ...
            if self.args.ours:
                outputs, tuples, regularize_loss = self.extractor(x=data_aug)
            else:
    ...
    def do_test_save(self, loader):
        ...
        for it, ((data, nouse, class_l), _, _) in enumerate(loader):
            ...
            z, tuple = self.extractor(data, train=False)

            # [ANDY]
            for i in range(self.args.num_extractors):
                dic_features[i] += torch.sum(tuple[f"feature_{i}"], 0)

        # [ANDY] save training features for adaptive inferencce
        if self.args.ours:
            if not os.exist('./features'):
                os.mkdir('features')
            for i in range(self.args.num_extractors):
                torch.save(dic_features[i] / total, f'./features/{self.args.task}/{i}')
    ...
    def do_training(self):
        ...
        for self.current_epoch in range(self.args.epochs):
            ...
            if self.results["test"][self.current_epoch] > current_high:
                ...
                torch.save(self.extractor.state_dict(), os.path.join('logs/model/', 'best.pth'))
